...
class GameOverSubstate extends MusicBeatSubstate
{
	...
	override function update(elapsed:Float)
	{
		...
		if (controls.BACK)
		{
			...
			Mods.loadTopMod();
			if (PlayState.isStoryMode) FlxG.switchState(() -> new funkin.states.StoryMenu());
			else FlxG.switchState(() -> new funkin.states.freeplay.FreeplayState());
	...
	function endBullshit(muted:Bool = false):Void
	{
		if (!isEnding)
		{
			isEnding = true;
			...
			FlxG.sound.music.stop();
			if (!muted) FlxG.sound.play(Paths.music(endSoundName));
