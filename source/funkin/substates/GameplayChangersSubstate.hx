...
import funkin.options.BaseOptionsMenu.AttachedFlxText;
...
class GameplayChangersSubstate extends MusicBeatSubstate
{
	...
	private var grpOptions:FlxTypedGroup<BaldiText>;
	...
	public function new()
	{
		...
		grpOptions = new FlxTypedGroup<BaldiText>();
		add(grpOptions);
		...
		{
			var optionText:BaldiText = new BaldiText(200, 360, 0, optionsArray[i].name);
			optionText.isMenuItem = true;
			// optionText.setScale(0.8);
			optionText.targetY = i;
			grpOptions.add(optionText);
			optionText.startPosition.y = FlxG.height / 2;
			...
			if (optionsArray[i].type == 'bool')
			{
				...
				var checkbox:BaldiCheckmark = new BaldiCheckmark(optionText.x - 105, optionText.y, optionsArray[i].getValue() == true);
				...
				{
					once = true;
					// checkbox.replaceColor(FlxColor.BLACK,FlxColor.WHITE);
				}
			}
			else
			{
				optionText.snapToPosition();
				var valueText:AttachedFlxText = new AttachedFlxText(Std.string(optionsArray[i].getValue()), optionText.width + 40);
